#define ASSERT_ABILITY_USABILITY		if(HAS_TRAIT(owner, TRAIT_HOLOPARA_BYSTANDER)) { to_chat(owner, "<span class='warning bold'>You can't do that right now!</span>"); return FALSE; }
#define ASSERT_ABILITY_USABILITY_SILENT	if(HAS_TRAIT(owner, TRAIT_HOLOPARA_BYSTANDER)) { return FALSE; }
