		...
		if(1, 2, 3)
			...
			if(affected_mob.CanSpreadAirborneDisease()) //don't spread germs if they covered their mouth
				if(active_disease.spread_flags != DISEASE_SPREAD_FALTERED) //Monkestation edit #186
					for(var/mob/living/exposed_mob in oview(spread_range, affected_mob)) //Monkestation edit #186
						if(is_source_facing_target(affected_mob, exposed_mob) && disease_air_spread_walk(get_turf(affected_mob), get_turf(exposed_mob))) //Monkestation edit #186
							exposed_mob.AirborneContractDisease(active_disease, TRUE) //Monkestation edit #186
