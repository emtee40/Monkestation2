...
/datum/reagent/water/holywater/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	...
	if(IS_CULTIST(affected_mob) || affected_mob.mind?.has_antag_datum(/datum/antagonist/clock_cultist))	//Monkestation Edit: Clock cult is cult
	...
	if(data["misc"] >= (25 SECONDS)) // 10 units
		...
		if(IS_CLOCK(affected_mob) && SPT_PROB(10, seconds_per_tick))
			...
			if(prob(10))
				affected_mob.visible_message(span_danger("[affected_mob] starts having a seizure!"), span_userdanger("You have a seizure!"))
				affected_mob.Unconscious(12 SECONDS)
				to_chat(affected_mob, "<span class='big_brass'>[pick("Your failure shall not delay my freedom.", "The blind will see only darkness.",
				"Then my ark will feed upon your vitality.", "Do not forget your servitude.")].</span>")
