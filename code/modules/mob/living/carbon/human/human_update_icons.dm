...
/mob/living/carbon/human/proc/apply_height_filters(mutable_appearance/appearance)
	...
	switch(get_mob_height())
		...
		if(HUMAN_HEIGHT_DWARF)
			appearance.add_filter("Gnome_Cut_Torso", 1, displacement_map_filter(cut_torso_mask, x = 0, y = 0, size = 1))
			appearance.add_filter("Gnome_Cut_Legs", 1, displacement_map_filter(cut_legs_mask, x = 0, y = 0, size = 1))
