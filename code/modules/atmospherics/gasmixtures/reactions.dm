	...
	if(reaction_energy)
		if(location)
			...
			if(prob(PERCENT(particle_chance)))
				location.fire_nuclear_particle()
			var/rad_power = max((FUSION_RAD_COEFFICIENT/instability) + 700, 15000) //Wow!
			radiation_pulse(location, rand(1,25), 9, 60, 2 SECONDS, 15, rad_power) // Nonmodular. Fixes fusion going wacko mode - makes it significantly harder to irradiate people through walls, and fixes the range issue.
		var/new_heat_capacity = air.heat_capacity()
