...
//	RegisterSignal(src, COMSIG_QDELETING, PROC_REF(on_delete)) MONKEYSTATION EDIT CHANGE OLD
	RegisterSignal(src, COMSIG_PARENT_QDELETING, PROC_REF(on_delete))// MONKEYSTATION EDIT CHANGE NEW - we dont have PR #75914
...
//	set_light(l_range = 2, l_power = 1.5, l_color = is_ready ? LIGHT_COLOR_BABY_BLUE : LIGHT_COLOR_FIRE, l_on = TRUE) MONKEYSTATION EDIT ORIGINAL - We have changed lights
	set_light(l_inner_range = 1, l_outer_range = 2, l_power = 1.5, l_color = is_ready ? LIGHT_COLOR_BABY_BLUE : LIGHT_COLOR_FIRE, l_on = TRUE) // MONKEYSTATION EDIT NEW
	return ..()
