 ...
 */
/mob/living/basic/holoparasite/proc/eligible_for_reset()
	. = FALSE
	if(!ckey)
		return TRUE
 ...
 */
/mob/living/basic/holoparasite/Logout()
	. = ..()
 ...
 */
/mob/living/basic/holoparasite/proc/reset(cooldown = TRUE, automatic = FALSE, self = FALSE)
	set waitfor = FALSE
	if(being_reset)
		return
	being_reset = TRUE
	...
	if(!length(candidates))
		to_chat(summoner.current, span_boldholoparasite("[color_name] could not be reset, as there were no eligible candidate personalities willing to take over!"))
		if(self)
			to_chat(src, span_boldholoparasite("Personality reset [span_danger("failed")]: no eligible candidate personalities."))
		return
	var/mob/dead/observer/new_player = pick(candidates)
	if(!new_player)
		to_chat(summoner.current, span_boldholoparasite("[color_name] could not be reset due to an error!"))
		if(self)
			to_chat(summoner.current, span_boldholoparasite("Personality reset [span_danger("failed")]: unknown error!"))
		return
	to_chat(src, span_big(span_boldholoparasite(self ? "A ghost took control of you, at your request." : "Your summoner reset you! Better luck next time!")))
	ghostize(can_reenter_corpse = FALSE)
	key = new_player.key
	to_chat(summoner.current, span_big(span_boldholoparasite("Personality reset for [color_name] succeeded!")))
