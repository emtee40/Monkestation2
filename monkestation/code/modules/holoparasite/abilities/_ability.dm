...
/**
 * Sets up parts of the ability related to stat thresholds and such.
 * This is just called by apply(), but this allows for cleaner, less crowsded code.
 */
/datum/holoparasite_ability/proc/setup_thresholds()
	SHOULD_CALL_PARENT(TRUE)
...
/datum/holoparasite_ability/proc/remove()
	...
	if(owner.summoner?.current)
		REMOVE_TRAITS_IN(owner.summoner.current, REF(src))
