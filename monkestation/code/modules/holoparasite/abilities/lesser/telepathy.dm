/datum/holoparasite_ability/lesser/telepathy
	...
	//var/datum/action/holoparasite/telepathy/telepathy_hud
...
/datum/holoparasite_ability/lesser/telepathy/proc/telepathy(mob/living/target, message, sanitize = TRUE)
	...
	to_chat(owner, span_holoparasite("You telepathically said: \"<span class='message'>[message]</span>\" to [span_name("[target]")]>."), type = MESSAGE_TYPE_RADIO, avoid_highlighting = TRUE)
	to_chat(target, span_holoparasite("[response_href][span_notice("You hear a strange, resonating voice in your head... ")]<span class='message'>[COLOR_TEXT(owner.accent_color, message)]</span>"), type = MESSAGE_TYPE_RADIO)
	...
	for(var/mob/dead/dead in GLOB.dead_mob_list)
		...
		to_chat(dead, span_holoparasite("[follow_link_user] [owner.color_name] Telepathy --> [follow_link_target] [span_name("[target]")] <span class='message'>[message]</span>"), type = MESSAGE_TYPE_RADIO)
...
/datum/holoparasite_ability/lesser/telepathy/Topic(href, list/href_list)
	...
	to_chat(owner, span_holoparasite("Telepathic response from [span_name("[usr]")]: <span class='message'>[message]</span>"), type = MESSAGE_TYPE_RADIO)
	log_directed_talk(usr, owner, message, LOG_SAY, "holoparasite telepathy response")
	//usr.create_chat_message(usr, /datum/language/metalanguage, list(owner), raw_message = message, spans = list("holoparasite"))
	for(var/mob/dead/observer/gost in GLOB.dead_mob_list)
		...
		to_chat(gost, span_holoparasite("[follow_link_user] [span_name("[usr]")] Telepathic Response --> [follow_link_owner] [owner.color_name] <span class='message'>[message]</span>"), type = MESSAGE_TYPE_RADIO)
	...
	if(!can_respond)
		if(iscarbon(responder))
			...
			if(carbon_responder.has_dna())
				...
				if(is_species(carbon_responder, /datum/species/jelly/stargazer))
					return TRUE
...
/*
/datum/action/holoparasite/telepathy
	name = "Telepathy"
...
/datum/action/holoparasite/telepathy/Initialize(_mapload, mob/living/basic/holoparasite/_owner, datum/holoparasite_ability/lesser/telepathy/_ability)
	. = ..()
	if(!istype(_ability))
...
/datum/action/holoparasite/telepathy/on_login()
	if(!text_overlay || !owner?.client)
		return
	owner.client.images |= text_overlay

/datum/action/holoparasite/telepathy/update_overlays()
	. = ..()
	if(!text_overlay)
...
/datum/action/holoparasite/telepathy/Click(location, control, params)
...
/datum/action/holoparasite/telepathy/should_be_transparent()
