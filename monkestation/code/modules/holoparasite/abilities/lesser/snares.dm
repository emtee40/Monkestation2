/datum/holoparasite_ability/lesser/snare
	...
	var/datum/action/holoparasite/snare/arm/arm_hud
	/// The HUD button for disarming snares.
	var/datum/action/holoparasite/snare/disarm/disarm_hud
...
/datum/action/holoparasite/snare
	var/datum/holoparasite_ability/lesser/snare/ability

/datum/action/holoparasite/snare/Initialize(_mapload, mob/living/basic/holoparasite/_owner, datum/holoparasite_ability/lesser/snare/_ability)
	. = ..()
	if(!istype(_ability))
		CRASH("Tried to make snare holoparasite HUD without proper reference to snare ability")
	ability = _ability

/datum/action/holoparasite/snare/arm
	name = "Arm Snare"
	desc = "Arm a surveillance snare below you, which will alert you whenever someone walks over it."
	icon_state = "snare:arm"

/datum/action/holoparasite/snare/arm/Click(location, control, params)
	if(ability.arming)
		return
	ability.try_arm_snare()

/datum/action/holoparasite/snare/arm/should_be_transparent()
	return length(ability.snares) >= HOLOPARA_MAX_SNARES

/datum/action/holoparasite/snare/arm/in_use()
	return ability.arming

/datum/action/holoparasite/snare/arm/update_overlays()
	. = ..()
	if(!text_overlay)
...
/datum/action/holoparasite/snare/disarm
	name = "Disarm Snare"
	desc = "Disarm a surveillance snare that you have setup."
	icon_state = "snare:disarm"

/datum/action/holoparasite/snare/disarm/should_be_transparent()
	return !length(ability.snares)

/datum/action/holoparasite/snare/disarm/Click(location, control, params)
...
/obj/effect/snare/proc/on_entered(datum/source, mob/living/crosser)
	...
	if(HAS_TRAIT(crosser, TRAIT_LIGHT_STEP) && prob(45))
		return
	var/mob/living/basic/holoparasite/owner = ability.owner
