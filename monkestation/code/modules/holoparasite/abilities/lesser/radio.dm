...
/datum/holoparasite_ability/lesser/misaka/setup_thresholds()
	. = ..()
	can_talk = master_stats.range >= 4
	binary = master_stats.potential >= 4
...
/datum/holoparasite_ability/lesser/misaka/notify_user()
	var/list/text = list(span_boldholoparasite("You are able to [can_talk ? "both hear and talk over" : "hear"] most radio channels!"))
