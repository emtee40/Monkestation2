/datum/holoparasite_ability/major/frenzy
	...
	/// The unique movespeed modifier attached to this ability.
	var/datum/movespeed_modifier/holopara_frenzy/modifier
...
/datum/holoparasite_ability/major/frenzy/New(datum/holoparasite_stats/_master_stats)
	. = ..()
	modifier = new
	modifier.id = "[modifier.id]-[REF(src)]"

/datum/holoparasite_ability/major/frenzy/Destroy()
	QDEL_NULL(modifier)
	return ..()
...
/datum/holoparasite_ability/major/frenzy/apply()
	...
	owner.AddComponent(/datum/component/after_image, rest_time = 0.5)
...
/datum/holoparasite_ability/major/frenzy/setup_thresholds()
	. = ..()
...
/datum/holoparasite_ability/major/frenzy/unregister_signals()
	. = ..()
	UnregisterSignal(owner, list(COMSIG_HOLOPARA_MANIFEST, COMSIG_HOLOPARA_RECALL, COMSIG_HOLOPARA_STAT, COMSIG_BASICMOB_PRE_ATTACK_RANGED))
...
/datum/holoparasite_ability/major/frenzy/proc/on_manifest()
	...
	summoner.add_movespeed_modifier(modifier, update = TRUE)
	summoner.AddComponent(/datum/component/after_image, rest_time = 0.5)
	ADD_TRAIT(summoner, TRAIT_IGNORESLOWDOWN, REF(src))
...
/datum/holoparasite_ability/major/frenzy/proc/on_recall()
	SIGNAL_HANDLER
	var/mob/living/summoner = owner.summoner.current
	summoner.remove_movespeed_modifier(modifier)
	var/datum/component/after_image = summoner.GetComponent(/datum/component/after_image)
	qdel(after_image)
	REMOVE_TRAIT(summoner, TRAIT_IGNORESLOWDOWN, REF(src))
	summoner.balloon_alert(summoner, "frenzy speed boost lost")

/datum/holoparasite_ability/major/frenzy/proc/on_ranged_attack(mob/living/basic/firer, atom/target, modifiers)
	SIGNAL_HANDLER
	ASSERT_ABILITY_USABILITY
	if(!istype(target) || !LAZYACCESS(modifiers, RIGHT_CLICK) || !CHECK_BITFIELD(owner.istate, ISTATE_HARM) || !COOLDOWN_FINISHED(src, rush_cooldown) || !owner.is_manifested())
		return
	...
	owner.forceMove(get_step(get_turf(target), get_dir(target, owner)))
	owner.next_move = 1
	...
	if(rush_knockback)
		...
		owner.visible_message(span_danger("[owner.color_name] violently rushes and attacks [target], flinging [target.p_them()] backwards!"), span_warning("We violently rush and attack [target], flinging [target.p_them()] backwards!"), ignored_mobs = list(target))
		...
		owner.visible_message(span_danger("[owner.color_name] violently rushes and attacks [target]"), span_warning("We violently rush and attack [target]!"), ignored_mobs = list(target))
...
/datum/holoparasite_ability/major/frenzy/proc/on_stat(datum/_source, list/tab_data)
	...
	if(!COOLDOWN_FINISHED(src, rush_cooldown))
		tab_data += "Frenzy Rush Cooldown: [COOLDOWN_TIMELEFT_TEXT(src, rush_cooldown)]"

/datum/movespeed_modifier/holopara_frenzy
	id = MOVESPEED_ID_HOLOPARA_FRENZY
	movetypes = ALL
	priority = 100
