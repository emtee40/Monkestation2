/datum/holoparasite_ability/major/assassin
	...
	var/datum/action/holoparasite/toggle_assassin/toggle_hud
...
/datum/holoparasite_ability/major/assassin/apply()
	...
	if(QDELETED(toggle_hud))
		toggle_hud = new(src)
		toggle_hud.ability = src
	toggle_hud.Grant(owner)

/datum/holoparasite_ability/major/assassin/remove()
	. = ..()
	if(!QDELETED(toggle_hud))
		toggle_hud.Remove(owner)

/datum/holoparasite_ability/major/assassin/setup_thresholds()
	. = ..()
	stealth_cooldown_time = round((7.5 SECONDS) / master_stats.potential, 0.5 SECONDS)
...
/datum/holoparasite_ability/major/assassin/unregister_signals()
	. = ..()
	UnregisterSignal(owner, COMSIG_HOLOPARA_STAT)
...
/datum/holoparasite_ability/major/assassin/proc/exit_assassin(forced = FALSE)
	...
	if(!forced)
		...
		if(owner.hud_used)
			var/datum/action/holoparasite/manifest_recall/mr_hud = locate() in owner.hud_used.static_inventory
...
/datum/action/holoparasite/toggle_assassin
...
/datum/action/holoparasite/toggle_assassin/Trigger()
	if(ability.assassin)
...
/datum/action/holoparasite/toggle_assassin/tooltip_content()
	. = ..()
