...
/datum/holoparasite_ability/major/spatial/unregister_signals()
	..()
	UnregisterSignal(owner, list(COMSIG_BASICMOB_PRE_ATTACK_RANGED, COMSIG_HOLOPARA_STAT))

/datum/holoparasite_ability/major/spatial/proc/on_ranged_attack(mob/living/basic/firer, atom/target, modifiers)
	SIGNAL_HANDLER
	ASSERT_ABILITY_USABILITY
	if(!COOLDOWN_FINISHED(src, spatial_rending_cooldown) || !LAZYACCESS(modifiers, RIGHT_CLICK) || !CHECK_BITFIELD(owner.istate, ISTATE_HARM) || owner.Adjacent(target) || !isturf(owner.loc))
		return
	...
	if(things_pulled)
		playsound(owner, 'monkestation/sound/holoparasites/rend.ogg', vol = 80, vary = TRUE) // blink lol
...
/datum/holoparasite_ability/major/spatial/proc/on_stat(datum/_source, list/tab_data)
	...
	if(!COOLDOWN_FINISHED(src, spatial_rending_cooldown))
		tab_data += "Spatial Rend Cooldown Remaining: [COOLDOWN_TIMELEFT_TEXT(src, spatial_rending_cooldown)]"
