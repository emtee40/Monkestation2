/datum/holoparasite_ability/weapon/dextrous
	...
	traits = list(TRAIT_ADVANCEDTOOLUSER)
	...
	var/atom/movable/screen/drop/drop
...
/datum/holoparasite_ability/weapon/dextrous/apply()
	...
	owner.melee_damage_lower = 6 + round((master_stats.damage - 1) * 0.8) // approximately the same as an average human's punch
	owner.melee_damage_upper = owner.melee_damage_lower
	owner.obj_damage = 0
	...
	owner.AddComponent(/datum/component/personal_crafting)
	owner.AddElement(/datum/element/dextrous, 2, /datum/hud/holoparasite)
	. = ..()

/datum/holoparasite_ability/weapon/dextrous/remove()
	owner.unequip_everything()
	owner.melee_damage_lower = initial(owner.melee_damage_lower)
	owner.melee_damage_upper = initial(owner.melee_damage_upper)
	...
	qdel(owner.GetComponent(/datum/component/personal_crafting))
	owner.RemoveElement(/datum/element/dextrous)
	. = ..()
...
/datum/holoparasite_ability/weapon/dextrous/proc/create_misc_hud(datum/hud/holoparasite/hud, list/huds_to_add)
	...
	var/atom/movable/screen/drop/drop = new
