 ...
 */
/mob/living/basic/holoparasite/proc/can_be_manifested()
	. = TRUE
 ...
 */
/mob/living/basic/holoparasite/proc/is_attached_to_summoner(permanently = FALSE, check_manifested = TRUE)
	if(QDELETED(summoner.current))
		return FALSE
 ...
 */
/mob/living/basic/holoparasite/proc/is_in_range(atom/target)
 ...
 */
/mob/living/basic/holoparasite/proc/is_summoner_dead()
	. = FALSE
	if(QDELETED(summoner.current))
		return TRUE
 ...
 */
/mob/living/basic/holoparasite/proc/is_manifested()
 ...
 */
/mob/living/basic/holoparasite/proc/has_matching_summoner(mob/living/other, include_summoner = TRUE)
	if(!istype(other))
		return FALSE
	if(other == src)
		return TRUE
	if(isholopara(other))
		var/mob/living/basic/holoparasite/other_holopara = other
 ...
 */
/mob/living/basic/holoparasite/proc/list_summoner_and_or_holoparasites(include_self = TRUE, include_summoner = TRUE)
	. = list()
	if(include_self)
		. += src
 ...
 */
/mob/living/basic/holoparasite/proc/is_light_on()
 ...
 */
/mob/living/basic/holoparasite/proc/give_accent_border(atom/target, size = 1)
	if(QDELETED(target))
		return
