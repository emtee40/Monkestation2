#define ADD_BARRIER_IMAGE(pos, direction) barrier_images += image('monkestation/icons/effects/effects.dmi', pos, "barrier", FLOAT_LAYER, direction)
...
/mob/living/basic/holoparasite/proc/cut_barriers()
	if(LAZYLEN(barrier_images))
		if(client)
			client.images -= barrier_images
		QDEL_LAZYLIST(barrier_images)

/mob/living/basic/holoparasite/proc/setup_barriers()
	cut_barriers()
