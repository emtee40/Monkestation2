...
/datum/holoparasite_builder/New(obj/item/_host, datum/holoparasite_theme/_theme, _max_points, _max_level, _uses, _debug_mode)
	...
	accent_color = GLOB.color_list_ethereal[pick(GLOB.color_list_ethereal)]
...
/datum/holoparasite_builder/ui_data(mob/user)
	. = list(
		...
		"validation" = list(
			...
			"color" = "valid", // TODO: [Shion Holoparas] color checks
...
/datum/holoparasite_builder/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	switch(action)
		...
		if("set:stat")
			...
			if(!istext(stat) || !length(stat) || !isnum(value))
				return
		...
		if("set:color")
			...
			if(!istext(color) || length(color) != 7)
				return
			var/new_accent_color = sanitize_hexcolor(color, desired_format = 6, include_crunch = TRUE, default = (length(accent_color) == 7 && accent_color != initial(accent_color)) ? accent_color : GLOB.color_list_ethereal[pick(GLOB.color_list_ethereal)])
			/* TODO: [Shion Holoparas] color checks
		...
		if("random")
			...
			to_chat(usr, "<span class='boldnotice'>Stats randomized.</span>")
			. = TRUE
	if(.)
...
/datum/holoparasite_builder/proc/enforce_ability_weapon(new_weapon_path, silent = FALSE)
	...
	to_chat(usr, "<span class='danger'>The <b>[saved_stats.ability.name]</b> ability forces you to use the <b>[forced_weapon.name]</b> weapon! You cannot choose a different weapon with this ability selected!</span>")
...
/datum/holoparasite_builder/proc/is_valid_ability(path, base_path)
	...
	if(!istype(ability) || !isnum(ability.cost)) // null cost means this ability is NOT obtainable through normal means.
		return FALSE
/**
...
/datum/holoparasite_builder/proc/spawn_holoparasite(mob/living/user)
	...
	*/
	...
	var/mob/living/basic/holoparasite/holoparasite = new(user, candidate.key, holopara_name, theme, accent_color, notes, user.mind, saved_stats)
...
/obj/item/holoparasite_creator/attack_self(mob/living/user)
	if(!user.mind)
		return
	if(isholopara(user))
		to_chat(user, "<span class='holoparasite'>[builder.theme.name] chains are not allowed.</span>")
		return
	if(user.has_holoparasites() && !allow_multiple)
		to_chat(user, "<span class='holoparasite'>You already have a [builder.theme.name]!</span>")
		return
...
/obj/item/holoparasite_creator/vv_edit_var(var_name, var_value)
	switch(var_name)
		...
		if(NAMEOF(src, max_points))
			if(!isnum(var_value) || var_value < 1)
				return FALSE
