...
/mob/living/proc/holoparasite_telepathy(msg, sanitize = TRUE)
	var/list/mob/living/basic/holoparasite/holoparas = holoparasites()
	...
	create_chat_message(src, null, holoparas + src, raw_message = msg, spans = list("holoparasite"))
...
/mob/living/basic/holoparasite/holoparasite_telepathy(msg, sanitize = TRUE)
	if(!summoner.current)
		return FALSE
	msg = trim(msg, max_length = MAX_MESSAGE_LEN)
	if(!length(msg))
	...
	create_chat_message(src, null, recipients, raw_message = msg, spans = list("holoparasite"))
	for(var/ghost in GLOB.dead_mob_list)
		var/link = FOLLOW_LINK(ghost, src)
...
/mob/living/basic/holoparasite/say(message, bubble_type, list/spans, sanitize, datum/language/language, ignore_spam, forced)
...
/mob/living/basic/holoparasite/radio(message, list/message_mods, list/spans, language)
	var/datum/holoparasite_ability/lesser/misaka/radio_noise = stats.has_lesser_ability(/datum/holoparasite_ability/lesser/misaka)
...
/mob/living/basic/holoparasite/binarycheck()
	var/datum/holoparasite_ability/lesser/misaka/radio_noise = stats.has_lesser_ability(/datum/holoparasite_ability/lesser/misaka)
