 ...
 */
/mob/living/basic/holoparasite/proc/set_summoner(datum/mind/new_summoner)
 ...
 */
/mob/living/basic/holoparasite/proc/set_holopara_name(new_name, silent = FALSE, internal = FALSE)
	. = TRUE
	if(!istext(new_name))
		return FALSE
 ...
 */
/mob/living/basic/holoparasite/proc/set_accent_color(new_color, silent = FALSE)
	if(!new_color)
		return FALSE
	var/old_accent_color = accent_color
	new_color = sanitize_hexcolor(new_color, desired_format = 6, include_crunch = TRUE, default = (length(old_accent_color) == 7 && old_accent_color != initial(accent_color)) ? old_accent_color : GLOB.color_list_ethereal[pick(GLOB.color_list_ethereal)])
...
/mob/living/basic/holoparasite/proc/set_theme(datum/holoparasite_theme/new_theme)
...
/mob/living/basic/holoparasite/proc/set_battlecry(new_battlecry, silent = FALSE)
	. = TRUE
