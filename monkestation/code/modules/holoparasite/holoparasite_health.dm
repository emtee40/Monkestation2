/mob/living/basic/holoparasite/update_health_hud()
	if(!hud_used?.healths)
		return
	var/mob/living/current = summoner?.current
	if(!QDELETED(current) && current.stat != DEAD)
		var/summoner_max_health = current.maxHealth
		var/health_amount = min(current.health, summoner_max_health - current.staminaloss)
...
/mob/living/basic/holoparasite/revive(full_heal_flags = NONE, excess_healing = 0, force_grab_ghost = FALSE)
	. = ..()
	if(.)
