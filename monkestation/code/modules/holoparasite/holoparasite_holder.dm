...
/datum/holoparasite_holder
	...
	var/list/mob/living/basic/holoparasite/holoparasites = list()
...
/datum/holoparasite_holder/proc/unregister_mind_signals()
	UnregisterSignal(owner, list(COMSIG_MIND_TRANSFERRED/*, COMSIG_MIND_JOIN_ANTAG_HUD, COMSIG_MIND_LEAVE_ANTAG_HUD*/))
 ...
 */
/datum/holoparasite_holder/proc/add_holoparasite(mob/living/basic/holoparasite/new_holopara)
 ...
 */
/datum/holoparasite_holder/proc/remove_holoparasite(mob/living/basic/holoparasite/holopara_to_remove)
...
/datum/holoparasite_holder/proc/transfer_holoparasites_to_body(mob/living/new_body)
	if(new_body.stat == DEAD && !HAS_TRAIT(new_body, TRAIT_NODEATH))
		return
	for(var/mob/living/basic/holoparasite/holopara as() in holoparasites)
 ...
 */
/datum/holoparasite_holder/proc/on_leave_antag_hud(datum/mind/_source)
	SIGNAL_HANDLER
...
/datum/holoparasite_holder/proc/on_login(mob/living/source)
	...
	if(!holopara_amt)
		return
	to_chat(source, span_big(span_holoparasite("You can use :[MODE_KEY_HOLOPARASITE] or .[MODE_KEY_HOLOPARASITE] to privately communicate with your holoparasite[holopara_amt > 1 ? "s" : ""]!")))
...
/datum/holoparasite_holder/proc/remove_all_tracking_huds()
	for(var/mob/living/basic/holoparasite/holopara as() in holoparasites)
		if(QDELETED(holopara.tracking_beacon))
			continue
