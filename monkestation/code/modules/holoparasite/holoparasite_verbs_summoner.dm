...
/mob/living/basic/holoparasite/update_holoparasite_verbs()
	return
...
/mob/living/proc/holoparasite_reset()
	...
	if(!holoparas)
		...
		return
	var/mob/living/basic/holoparasite/holopara = tgui_input_list(src, "Which holoparasite would you like to reset?", "Holoparasite Reset", holoparas)
	if(!holopara)
		return
...
/mob/living/proc/holoparasite_set_notes()
	...
	if(!new_notes)
		return
	for(var/mob/living/basic/holoparasite/holopara as() in holoparas_to_set)
...
/mob/living/proc/holoparasite_lock()
	...
	if(holder.locked)
		...
		for(var/mob/living/basic/holoparasite/holopara as() in holder.holoparasites)
