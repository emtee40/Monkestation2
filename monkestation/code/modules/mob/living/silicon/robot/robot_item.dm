...
/datum/design/borg_upgrade_clamp
	...
	materials = list(/datum/material/titanium = MINERAL_MATERIAL_AMOUNT * 2, /datum/material/gold = MINERAL_MATERIAL_AMOUNT * 0.5, /datum/material/bluespace = MINERAL_MATERIAL_AMOUNT * 0.2)
...
/obj/item/paperplane/syndicate/hardlight/throw_impact(atom/hit_atom, datum/thrownthing/throwingdatum)
	. = ..()
	qdel(src)
...
/obj/item/borg/paperplane_crossbow/proc/shoot(atom/target, mob/living/user, params)
	...
	var/obj/item/paperplane/syndicate/hardlight/plane_to_fire = new(get_turf(src.loc))
...
/obj/item/stack/wrapping_paper/xmas/cyborg/use(used, transfer, check)
	check = FALSE
	return ..()
