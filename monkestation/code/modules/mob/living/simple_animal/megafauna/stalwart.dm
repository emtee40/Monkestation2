...
/mob/living/simple_animal/hostile/megafauna/stalwart
	...
	crusher_loot = list(/obj/structure/closet/crate/necropolis/stalwart/crusher)
	loot = list(/obj/structure/closet/crate/necropolis/stalwart)
	death_message = "erupts into blue flame, and screeches before violently shattering."
	death_sound = 'sound/magic/castsummon.ogg'
...
/mob/living/simple_animal/hostile/megafauna/stalwart/proc/stalnade(set_angle)
	...
	var/angle_to_target = dir2angle(src, target_turf) // fuck up, maybe. replaced X with (dir2angle)
	if(isnum(set_angle))
...
/mob/living/simple_animal/hostile/megafauna/stalwart/proc/enrage(mob/living/victim)
	if(ishuman(victim))
