///ANIME ACCESSORIES GO HERE

/// Anime Horns

/datum/sprite_accessory/anime_horns
	icon = 'monkestation/icons/mob/anime/anime_horns.dmi'
	color_src = ANIME

/datum/sprite_accessory/anime_horns/frills
	name = "Frills"
	icon_state = "frills"

/datum/sprite_accessory/anime_horns/oni
	name = "Oni Horns"
	icon_state = "oni"

/datum/sprite_accessory/anime_horns/demon
	name = "Demon Horns"
	icon_state = "demon"

/datum/sprite_accessory/anime_horns/tallhorns
	name = "Tall Horns"
	icon = 'monkestation/icons/mob/anime/anime_ears32x48.dmi'
	icon_state = "tallhorns"

/datum/sprite_accessory/anime_horns/antlers
	name = "Antlers"
	icon_state = "antlers"

/datum/sprite_accessory/anime_horns/bigantlers
	name = "Big Antlers"
	icon = 'monkestation/icons/mob/anime/anime_horns32x48.dmi'
	icon_state = "bigantlers"

/// Anime Ears

/datum/sprite_accessory/anime_ears
	icon = 'monkestation/icons/mob/anime/anime_ears.dmi'
	color_src = ANIME

/datum/sprite_accessory/ears/none
	name = "None"
	icon_state = "None"

/datum/sprite_accessory/ears/pony
	name = "Pony Ears"
	icon_state = "pony"
	hasinner = TRUE

/datum/sprite_accessory/ears/kitsune
	name = "Kitsune Ears"
	icon_state = "kitsune"

/datum/sprite_accessory/ears/fox
	name = "Fox Ears"
	icon_state = "fox"
	hasinner = TRUE

/datum/sprite_accessory/ears/cat
	name = "Cat Ears"
	icon_state = "cat"
	hasinner = TRUE

/datum/sprite_accessory/ears/rabbit
	name = "Rabbit Ears"
	icon = 'monkestation/icons/mob/anime/anime_ears32x48.dmi'
	icon_state = "rabbit"

/datum/sprite_accessory/ears/mouse
	name = "Mouse Ears"
	icon_state = "mouse"
	hasinner = TRUE

/datum/sprite_accessory/ears/playbunny
	name = "Bunny Ears"
	icon = 'monkestation/icons/mob/anime/anime_ears32x48.dmi'
	icon_state = "playbunny"

/// Anime Chest

/datum/sprite_accessory/anime_middle
	icon = 'monkestation/icons/mob/anime/anime_middle.dmi'
	color_src = ANIME

/datum/sprite_accessory/anime_middle/none
	name = "None"
	icon_state = "none"

/datum/sprite_accessory/anime_middle/bee
	name = "Bee Wings"
	icon_state = "bee"

/datum/sprite_accessory/anime_middle/fairy
	name = "Fairy Wings"
	icon_state = "fairy"

/datum/sprite_accessory/anime_middle/tentacle
	name = "Tentacles"
	icon_state = "tentacle"

/datum/sprite_accessory/anime_middle/turtle
	name = "Turtle Shell"
	icon_state = "turtle"

/datum/sprite_accessory/anime_middle/demon
	name = "Demon Wings"
	icon_state = "demon"

/datum/sprite_accessory/anime_middle/angel
	name = "Angel Wings"
	icon_state = "angel"

/datum/sprite_accessory/anime_middle/seraphim
	name = "Seraphim Wings"
	icon_state = "seraphim"

/// Anime Tails

/datum/sprite_accessory/tails/anime_bottom
	icon = 'monkestation/icons/mob/anime/anime_bottom.dmi'
	color_src = ANIME

/datum/sprite_accessory/tails/anime_bottom/none
	name = "None"
	icon_state = "none"

/datum/sprite_accessory/tails/anime_bottom/bunny
	name = "Bunny Puff"
	icon_state = "playbunny"

/datum/sprite_accessory/tails/anime_bottom/cat
	name = "Cat Tail"
	icon_state = "cat"

/datum/sprite_accessory/tails/anime_bottom/corgi
	name = "Corgi Puff"
	icon_state = "corgi"

/datum/sprite_accessory/tails/anime_bottom/demon
	name = "Demon Tail"
	icon_state = "demon"

/datum/sprite_accessory/tails/anime_bottom/feathers
	name = "Feathers"
	icon_state = "feathers"

/datum/sprite_accessory/tails/anime_bottom/fox
	name = "Fox Tail"
	icon_state = "fox"

/datum/sprite_accessory/tails/anime_bottom/kitsune
	name = "Kitsune Tail"
	icon_state = "kitsune"
	hasinner = TRUE

/datum/sprite_accessory/tails/anime_bottom/nine_tails
	name = "Nine Tailed Fox"
	icon_state = "nine_tails"
	hasinner = TRUE

/datum/sprite_accessory/tails/anime_bottom/pony
	name = "Pony Tail"
	icon_state = "pony"

/datum/sprite_accessory/tails/anime_bottom/rabbit
	name = "Rabbit Tail"
	icon_state = "rabbit"

/datum/sprite_accessory/tails/anime_bottom/shark
	name = "Shark Tail"
	icon_state = "shark"

/datum/sprite_accessory/tails/anime_bottom/squirrel
	name = "Squirrel Tail"
	icon_state = "squirrel"

/datum/sprite_accessory/tails/anime_bottom/wolf
	name = "Wolf Tail"
	icon_state = "wolf"


