/datum/symptom/rodman_syndrome
	...
	resistance = -2
	stage_speed = 2
	transmittable = -4 	//Prevents airborne spread, on-contact is the highest possible
	...
	threshold_descs = list(
		"Resistance 10" = "The symptom now causes restoration of neural tissue in more traditional areas of the brain.",
		"Transmission 8" = "The symptom grows in intensity, causing the afflicted to begin grunting and screeching more often and with higher volume.",
		"Stealth+Stage Speed" = "with 2 stealth and 12 stage speed the symptom got a chance of granting ghost roles to living monkeys"
		)
...
/datum/symptom/rodman_syndrome/Start(datum/disease/advance/A)
	...
	if((A.totalStealth() >= 2) && (A.totalStageSpeed() >= 12))
		awaken_monkey = TRUE
		...
		if(4, 5)
			...
			if(!ishuman(M) && awaken_monkey && is_monkey) //Confirm they were a monkey from the start, currently a monkey and that the threshold is hit.
				...
				if(!M.mind && isliving(M)) //Confirm they are mindless and alive
					...
					notify_ghosts("[M] Monke time!", null, enter_link = "<a href=?src=[REF(M)];activate=1>(Click to play)</a>", source = M, action = NOTIFY_ORBIT, ignore_key = POLL_IGNORE_SENTIENCE_POTION)
					M.mind_initialize()
