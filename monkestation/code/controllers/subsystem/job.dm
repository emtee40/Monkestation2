...
/datum/controller/subsystem/job/proc/pick_desired_roundstart()
	...
	message_admins("p_d_r ACTUAL ROLESETS, [english_list(valid_rolesets)]")

	if(SSgamemode.current_roundstart_event && (SSgamemode.current_roundstart_event in valid_rolesets))
		message_admins("p_d_r failed, SSgamemode.current_roundstart_event in valid_rolesets")
		return

	if(!length(valid_rolesets))
		message_admins("p_d_r failed, no valid_rolesets")
		return

	SSgamemode.current_roundstart_event = pick_weight(valid_rolesets)
	message_admins("p_d_r pass, Selected Roleset: [SSgamemode.current_roundstart_event]")
